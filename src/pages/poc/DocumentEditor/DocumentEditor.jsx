...
export default function DocumentEditor() {
  ...
  const effectiveDoc = useMemo(
    () =>
      mergeChatWorkflowHighlights(liveDoc, chatWorkflowExtras, {
        /** From workflow chat: replace stored POC lists only when chat actually sent section/field targets. */
        replacePocHighlights: chatHasHighlightPayload,
      }),
    [liveDoc, chatWorkflowExtras, chatHasHighlightPayload],
  )
  ...
  const handleSaveDraft = () => {
    ...
    const ex = chatWorkflowExtras
    const fromChat =
      ex && (ex.sections?.length || ex.fields?.length)
        ? {
            poc_updated_sections: uniqLabels(ex.sections || []),
            poc_updated_fields: uniqLabels(ex.fields || []),
          }
        : {}
    updateDoc(doc.id, {
      status: 'draft',
  ...
  const handleSubmit = () => {
    ...
    const ex = chatWorkflowExtras
    const chatHighlightPersist =
      ex && (ex.sections?.length || ex.fields?.length)
        ? {
            poc_updated_sections: uniqLabels([...(latest.poc_updated_sections || []), ...(ex.sections || [])]),
            poc_updated_fields: uniqLabels([...(latest.poc_updated_fields || []), ...(ex.fields || [])]),
          }
        : {}
  ...
  return (
    <Layout>
      ...
      {inViewMode ? (
        <DocumentReadOnlySteps doc={effectiveDoc} step={currentStep} precisePocHighlightsOnly={chatHighlightSession} />
      ) : (
        <>
      {currentStep === 1 && (
        <KnowledgeArea
          ...
          stepHighlight={{
            ...
            poc:
              ...
              (!chatHighlightSession && isReviewerHighlightingWholeStep(1, pocSets.sections)),
          }}
        />
      )}
